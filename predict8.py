...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = '/gpu:7'
import sys
...
#    role = sagemaker.get_execution_role()
    ...
    predict_data = pd.read_csv('s3://bigonelab-data-transfer-to-tokyo/nlp/dy_ec/final_predict/sample/pre_sample_8.csv', dtype = {'spu_id':'string'})
    predict_data['spu_name']=predict_data['spu_name'].str.replace(',','')
    pd.DataFrame(predict_data['spu_name']).to_csv('s3://bigonelab-data-transfer-to-tokyo/nlp/dy_ec/final_predict/sample/pre_sample_8.txt',header=None,index=False)
    ...
    sentences = []
    with open('s3://bigonelab-data-transfer-to-tokyo/nlp/dy_ec/final_predict/sample/pre_sample_8.txt','r') as f:
        for line in f:
            sentences.append(line)
    print(len(sentences))
    df = pd.DataFrame(columns=['spu_name','label','batch_id'])
    df.to_csv('s3://bigonelab-data-transfer-to-tokyo/nlp/dy_ec/final_predict/sample/result_pre_sample_8.csv',index=False)
    with open('s3://bigonelab-data-transfer-to-tokyo/nlp/dy_ec/final_predict/sample/result_pre_sample_8.csv','a') as fd:
        i = 1
